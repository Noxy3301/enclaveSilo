...
constexpr std::uint64_t get_constant_for_non_uniform_random(std::uint64_t A,
     ...
     * varied without altering performance. The same C value, per field (C_LAST,
     * C_ID, and OL_I_ID), must be used by all emulated terminals.
     *
     * In order that the value of C used for C_LAST does not alter performance
     * the following must be true: Let C-Load be the value of C used to generate
     * C_LAST when populating the database. C-Load is a value in the range of
     * [0..255] including 0 and 255.
     *
     * Let C-Run be the value of C used to generate C_LAST for the measurement
     * run. Let C-Delta be the absolute value of the difference between C-Load
     * and C-Run. C-Delta must be a value in the range of [65..119] including
     * the values of 65 and 119 and excluding the value of 96 and 112.
     */
 ...
 * returned value is the length of the c_last name excluding the last null
 * character.
 */
...
struct IsOriginal {
  ...
  public:
    IsOriginal(std::size_t nr_total, std::size_t nr_original)
        ...
        // CAUSION: nr_original is too large. the following code will be very
        // very slow.
